...
SimulationResult Simulation2::run(const Dataset &dataset) {
    ...
    return {remainingOrders, resultVans,    vansUsed,     ordersDispatched,
            ordersForTheDay, vansForTheDay, deliveryTime, cost,
            reward};
}

SimulationResult Simulation2::run2DVSBPP(const Dataset &dataset) {

    /*
     * All the code written below is based on the article found at
     * https://www.sciencedirect.com/science/article/pii/S1572528605000216#sec5
     */
     ...
     *
     * Time complexity:
     * - Best case: O(|O|) -> if the first bin in the vector can contain every
     * item individually
     * - Worst case: O(|O|*|V|) -> if the last bin in the vector can contain
     ...
     *
     * Space complexity: O(|V|) -> creation of a linked list to perform the
     * algorithm.
     *
     * Time complexity:
     ...
     *
     * Since this algorithm is being backed by a linked list, item removal is
     * constant time, thus not introducing any overhead
     */
    std::list<Van> vanList(vans.begin(),
                           vans.end()); // Space Complexity: O(|O|)
    ...
    while (van1 != vanList.end()) {
        ...
        // These 2 lines are not related to the bin removal algorithm but can
        // save us some iterations later on
        if (maxVol < van1->getMaxVolume())
            maxVol = van1->getMaxVolume();
        if (maxWeight < van1->getMaxWeight())
            maxWeight = van1->getMaxWeight();

        if (van2->getCost() >= van1->getCost() &&
            van2->getMaxWeight() <= van1->getMaxWeight() &&
            van2->getMaxWeight() <=
                van1->getMaxWeight()) { // we can remove van2 because we know
                                        // that there are optimal solutions
